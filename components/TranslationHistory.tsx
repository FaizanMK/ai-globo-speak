...
async function TranslationHistory() {
  try {
    ...
    console.log(`Fetching translation history from URL: ${url}`);
    ...
    if (!response.ok) {
      console.error(
        `Error fetching translation history: ${response.statusText}`
      );
      throw new Error(
        `Failed to fetch translation history: ${response.status}`
      );
    }
    ...
    console.error("Error in TranslationHistory component:", error);
    throw error;
  }
}
